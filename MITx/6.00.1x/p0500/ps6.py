...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for i in range(26-shift):
            ...
            shift_dict[string.ascii_uppercase[i]] = string.ascii_uppercase[i+shift]
        for i in range(26-shift, 26):
            """
            比如shift=1时，range为(25,26)，仅仅有25的取值。也就是只有25位的z要特殊考虑。
            第25位的z经shift变为第0位的a。
            dict[char[25]] = char[0]
            
            shift=2时，range为(24,26)。只有24位的y和25位的z要特殊考虑。
            dict[char[24]] = char[0]
            dict[char[25]] = char[1]
            
            规律就是dict[char[i]] = char[i-(26-shift)]
            """
            shift_dict[string.ascii_lowercase[i]] = string.ascii_lowercase[i-(26-shift)]
            shift_dict[string.ascii_uppercase[i]] = string.ascii_uppercase[i-(26-shift)]
        return shift_dict
    ...
    def apply_shift(self, shift):
        ...
        for i in range(len(message_text)):
            try:
                shifted_text.append(shift_dict[message_text[i]])
            except KeyError:
                shifted_text.append(message_text[i])
        return ''.join(shifted_text)

# debug use
def mini_debug1(text, shift):
    print("origin: ", text)
