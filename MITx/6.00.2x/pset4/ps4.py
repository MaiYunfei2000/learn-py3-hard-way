...
# models = generate_models([1961, 1962, 1963],[4.4,5.5,6.6],[1, 2])
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        plt.plot(x, estimated, 'r-',
                 label = 'Fit of degree' + str(len(model))\
                 + ', R² = ' + str(round(error, 5)))
        plt.title('The temperature changes with the year')
        plt.xlabel('Year')
...
x2 = INTERVAL_2
y = []
# compute annual temperature in Boston in each year
for year in INTERVAL_1:
